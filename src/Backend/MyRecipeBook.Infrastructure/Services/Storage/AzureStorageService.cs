...
public class AzureStorageService : IBlobStorageService
{
    ...
    public async Task Upload(User user, Stream file, string fileName)
    {
        ...
        var blobCliente = container.GetBlobClient(fileName);

        await blobCliente.UploadAsync(file, overwrite:true);
    }
    ...
    public async Task<string> GetFileUrl(User user, string fileName)
    {
        ...
        var containerCliente = _blobServiceClient.GetBlobContainerClient(containerName);
        var exist = await containerCliente.ExistsAsync();
        if(exist.Value.IsFalse())
            return string.Empty;

        var blobClient = containerCliente.GetBlobClient(fileName);
